...
// check if a matrix generated by a combination of three indices (for the high
// bits) is an inverse of the original matrix
...
// for a given matrix, find the inverse by bruteforcing all possible
// combinations that can be used to generate a "valid" binary circulant matrix
...
// for each such matrix, see if an inverse can be found by iterating overall the
// other combinations
